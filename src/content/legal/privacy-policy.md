...
| 음성 데이터 | AI 할 일 생성 시 음성 인식 (기기 내 처리, 서버 미전송) |
| 생체인증 결과 | 앱 잠금 해제 (생체정보 자체는 기기에만 저장, 인증 성공/실패 여부만 처리) |
| 타임존 정보 (IANA 형식) | 알림 및 리마인더 시간대 설정 |

### 2-3. 소셜 로그인(OAuth) 시 제3자로부터 제공받는 정보
...
| 서비스 제공 | 할 일 관리, 캘린더(주간/월간), 친구 공유, 콕 찌르기, 응원, 주간 성취 배지 등 핵심 기능 제공. 공개 설정된 할 일은 친구에게 표시되며, 콕 찌르기·응원 메시지는 상대방에게 전달됩니다. |
| AI 기능 제공 | 할 일 자동 파싱, 추천 등 AI 기반 기능 |
| 구독 및 결제 관리 | 유료 구독 처리, 결제 상태 관리, 영수증 검증 |
| 알림 발송 | 할 일 리마인더, 소셜 알림, 서비스 공지 |
...
- 회원 탈퇴 시, 위 법정 보관 기간이 남아 있는 정보를 제외한 개인정보는 지체 없이 파기합니다.
...
1. 서비스는 AI 기반 할 일 자동 파싱 및 추천 기능을 제공하기 위해 회원의 할 일 텍스트를 외부 AI 서비스(Google Gemini API)로 전송합니다.
2. 전송되는 데이터는 할 일 텍스트에 한정되며, 이메일·이름 등 회원을 직접 식별할 수 있는 정보는 전송하지 않습니다.
...
1. 서비스는 AI를 활용하여 다음과 같은 **자동화된 결정**을 수행할 수 있습니다.
...
2. 회원가입 시 생년월일 확인을 통해 만 14세 미만 이용자를 식별하며, 해당 이용자의 가입을 차단합니다.
...
1. 본 개인정보처리방침은 **2026년 2월 28일**부터 시행합니다.
2. 이전 개인정보처리방침(2026년 2월 10일 시행)은 본 방침 시행과 동시에 효력을 상실합니다.
